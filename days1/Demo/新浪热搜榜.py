...
# import xlwt
# from days1.文件.excel文件存储 import workbook
#
# headers = {
...
#     hot = hot_elements[0] if hot_elements else "0"  # Provide default value if not found
...
#
# keys =list(host[0].keys())
# for i in range(len(keys)):
#     sheet1.write(0, i, keys[i])
# for row in range(1,len(host)+1,1):
#     for col,key in zip(range(len(keys)),keys):
#         sheet1.write(row,col,host[row-1][key])
# workbook.save("数据存储.xls")
#
#
import requests
from lxml import etree
